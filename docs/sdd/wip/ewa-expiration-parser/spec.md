...
5. devuelva un archivo Excel descargable con dos columnas: `Nombre` y `Fecha`.
...
- Intento controlado de procesamiento de `.doc` legado mediante conversion temporal en Windows.
...
- Generacion de un archivo Excel `.xlsx` con columnas `Nombre` y `Fecha`.
...
2. Un analista carga un `.doc` legado y el sistema intenta convertirlo y analizarlo; si no hay soporte, falla de forma controlada.
...
2. El sistema debe intentar procesar `.doc` y fallar de forma controlada si no hay conversion disponible.
...
9. El Excel final debe contener exactamente dos columnas: `Nombre` y `Fecha`.
...
- `.doc` en una maquina sin Microsoft Word disponible para conversion.

## Impacto tecnico

### Arquitectura

- `app/api/` expondra el endpoint FastAPI.
- `app/parsers/` se enfocara en extraer texto desde Word.
- `app/services/document_intelligence.py` definira la interfaz y proveedores de IA.
- `app/services/ewa_analysis_service.py` orquestara extraccion, IA, normalizacion, deduplicacion y Excel.
- `app/services/excel_service.py` seguira dedicado a la exportacion.
- `app/utils/` alojara utilidades de normalizacion de fechas.
- `app/models/` definira contratos de dominio para hallazgos crudos y normalizados.
...
- `.doc` legado dependera de conversion temporal en Windows si Microsoft Word esta disponible.
